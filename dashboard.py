...
#create_sum_order_items() bertanggung jawab untuk menyiapkan sum_orders_items
...
#create_bygender() digunakan untuk menyiapkan bygender
def create_bygender(df):
...
#create_byage() merupakan helper function yang digunakan untuk menyiapkan byage
def create_byage(df):
    byage = df.groupby(by="age_group").customer_id.nunique().reset_index()
    byage.rename(columns={
        "customer_id": "customer_count"
    }, inplace=True)
    byage['age_group'] = pd.Categorical(byage['age_group'], ["Youth", "Adults", "Seniors"])
    
    return byage

#create_bystate() digunakan untuk menyiapkan bystate
...
#create_rfm() bertanggung jawab untuk menghasilkan rfm
def create_rfm(df):
...
#Kolom order_date akan menjadi kunci dalam pembuatan filter. Nah, untuk ini, kita perlu mengurutkan DataFrame berdasarkan order_date serta memastikan kedua kolom tersebut bertipe datetime
datetime_columns = ["order_date", "delivery_date"]
all.sort_values(by="order_date", inplace=True)
...
min_date = all["order_date"].min()
max_date = all["order_date"].max()
...
main = all[(all["order_date"] >= str(start_date)) & 
                (all["order_date"] <= str(end_date))]

daily_orders = create_daily_orders(main)
sum_order_items = create_sum_order_items(main)
bygender = create_bygender(main)
byage = create_byage(main)
bystate = create_bystate(main)
rfm = create_rfm(main)

#Melengkapi Dashboard dengan Berbagai Visualisasi Data
st.header('Dicoding Collection Dashboard :sparkles:')

#informasi total order dan revenue dalam bentuk metric() yang ditampilkan menggunakan layout columns()
st.subheader('Daily Orders') #daily order
...
#informasi tentang performa penjualan dari setiap produk, kita akan menampilkan 5 produk paling laris dan paling sedikit terjual melalui sebuah visualisasi data
...
sns.barplot(x="quantity_x", y="product_name", data=sum_order_items.head(5), palette=colors, ax=ax[0])
...
sns.barplot(x="quantity_x", y="product_name", data=sum_order_items.sort_values(by="quantity_x", ascending=True).head(5), palette=colors, ax=ax[1])
...
with col1:
    ...
    sns.barplot(
        y="customer_count", 
        x="gender",
        data=bygender.sort_values(by="customer_count", ascending=False),
        palette=colors,
        ax=ax
    )
...
#RFM (Recency, Frequency, & Monetary), kita akan menampilkan nilai average atau rata-rata dari ketiga parameter tersebut menggunakan widget metric()
